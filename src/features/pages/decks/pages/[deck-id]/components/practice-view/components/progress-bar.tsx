...
function ProgressBar({ current, total }: ProgressBarProps): ReactTypes.JSXElement {
	...
	const classes = {
		...
		barLabel: cn(
			"absolute inset-0 flex items-center justify-center text-xs font-bold text-black",
			percentage >= 60 && "text-white",
		),
		count: "text-muted-foreground w-12 shrink-0 text-right text-sm font-semibold tabular-nums",
	};
